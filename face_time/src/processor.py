...
from face_time.src.logger import Logger, logger_process
...
def handle_one_picture(
    ...
    msg = f"Child {process.name} starting..."
    Logger.info(msg)
...
def handle_pictures_with_mp(
    ...
    thread = Thread(target=logger_process, args=(log_queue, eel.putMessageInOutput))
    thread.start()
